...
export default function TourPDFPage() {
    ...
    const handleDirectDownload = async () => {
        ...
        try {
            const response = await api.post('/leads/convert-to-pdf', {
                leadId,
                data,
                fileName: `Tour-Quotation-${data.quoteNumber || leadId}`
            }, {
                responseType: 'blob'
            });
            ...
            link.href = url;
            link.setAttribute('download', `Tour-Quotation-${data.quoteNumber || leadId}.pdf`);
    ...
    const handleDownloadWord = async () => {
        ...
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `Tour-Quotation-${data.quoteNumber || leadId}.docx`);
            document.body.appendChild(link);
            link.click();
            link.remove();
